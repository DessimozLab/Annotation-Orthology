...
def CreatePrimaryTranscriptsFile(fn, dout, gene_name_fn, q_use_original_accession_line, extract_transcript):
    ...
    outdic = dout + os.path.basename(fn).split('.')[0] + ".p"
        ...
        while iLine < N:
            ...
            while iLine < N:
                ...
                if line.startswith(">"):
                    ...
                    break
                outfile.write(line + "\n")
    with open(outdic, "wb") as outfp:
        pickle.dump(genetotrans, outfp)
...
def main(args=None):
    ...
    dout = os.path.dirname(os.path.abspath(fn)) + "/genetotranscripts/"
    ...
    if len(sys.argv) == 3:
        ...
        CreatePrimaryTranscriptsFile(fn, dout, gene_name_function_name, q_use_original_accession_line, extract_transcript)
